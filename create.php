...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nome_err) && empty($preco_err) && empty($imagem_err) && empty($quantidade_err)) {
        $sql = "INSERT INTO produtos (nome, preco, imagem, quantidade) VALUES (?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "sdsi", $nome, $preco, $imagem, $quantidade);

	    if (mysqli_stmt_execute($stmt)) {

		// Registra log após inserção
    		$detalhes_log = "Produto adicionado: Nome='$nome', Preço='$preco', Imagem='$imagem', Quantidade='$quantidade'";
    		$acao_log = "Criação de Produto";

    		$log_sql = "INSERT INTO logs (acao, detalhes) VALUES (?, ?)";
    		if ($log_stmt = mysqli_prepare($link, $log_sql)) {
        		mysqli_stmt_bind_param($log_stmt, "ss", $acao_log, $detalhes_log);
        		if (!mysqli_stmt_execute($log_stmt)) {
            			echo "Erro ao inserir log: " . mysqli_error($link);
        		}
        		mysqli_stmt_close($log_stmt);
    			} else {
        			echo "Erro ao preparar statement de log: " . mysqli_error($link);
    			}
